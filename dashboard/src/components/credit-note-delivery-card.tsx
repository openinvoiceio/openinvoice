...
import { DeliveryMethodEnum, type CreditNote } from "@/api/models";
import {
...
export function CreditNoteDeliveryCard({
  creditNote,
}: {
  ...
  const defaultRecipients = emailsToRecipientTags(creditNote.recipients);
  const form = useForm<FormValues>({
    resolver: zodResolver(schema),
    defaultValues: {
      delivery_method: creditNote.delivery_method,
  ...
  const { mutateAsync, isPending } = useUpdateCreditNote({
    mutation: {
      onSuccess: async ({ id }) => {
        await Promise.all([
          ...
          queryClient.invalidateQueries({
            queryKey: getPreviewCreditNoteQueryKey(id),
          }),
        ]);
      },
      onError: (error) => {
      ...
      <form>
        <FormCard className="pb-4">
          <FormCardHeader>
            ...
            <FormCardDescription>
              Choose how this credit note is delivered to your customer
            </FormCardDescription>
          </FormCardHeader>
          <FormCardContent>
            <FormField
                    ...
                    >
                      <div className="border-input has-data-[state=checked]:border-primary/50 relative flex w-full items-start gap-2 rounded-md border p-4 shadow-xs outline-none">
                        ...
                        <div className="grid grow gap-2">
                          ...
                          <p className="text-muted-foreground text-xs">
                            You will need to send the credit note to the
                            customer manually.
                          </p>
                        </div>
                      </div>
                      <div className="border-input has-data-[state=checked]:border-primary/50 relative flex w-full items-start gap-2 rounded-md border p-4 shadow-xs outline-none">
                        ...
                        <div className="grid grow gap-2 peer-disabled:opacity-50">
                          ...
                          <p className="text-muted-foreground text-xs">
                            Credit note will be automatically sent to recipients
                            when finalized.
                          </p>
                        </div>
                      </div>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                      Emails that will receive the credit note when it is
                      issued.
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
