...
export function TaxRateCreateSheet({
  name,
  onSuccess,
}: {
  name?: string;
  onSuccess?: (tax: TaxRate) => void;
}) {
  const formId = useId();
  ...
  const { isPending, mutateAsync } = useCreateTaxRate({
    mutation: {
      onSuccess: async (tax) => {
  ...
  return (
    <FormSheetContent>
      <FormSheetHeader>
        <FormSheetTitle>Create Tax Rate</FormSheetTitle>
        <FormSheetDescription>
          Define a new tax rate for your account.
        </FormSheetDescription>
      </FormSheetHeader>
      <Form {...form}>
