...
import {getVerificationCode} from "../utils/mailpit";
...
test.describe("Signup", () => {
    ...
    test("Cannot signup with numeric password", async ({page}, {title}) => {
        ...
        await expect(page.getByText("Password cannot be entirely numeric.")).toBeVisible();
    })
    ...
    test("Cannot verify email with expired code", async ({page, request}, {title}) => {
        const email = generateEmail(title)
        await page.getByLabel("Email").fill(email);
        await page.getByLabel("Password").fill(VALID_PASSWORD);
        await page.getByRole("button", {name: "Signup"}).click();
        ...
        await expect(page.getByText("Verification code expired. Please login again.")).toBeVisible();
    })
