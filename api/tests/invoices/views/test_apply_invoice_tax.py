...
def test_apply_invoice_tax(api_client, user, account):
    ...
    response = api_client.post(
        f"/api/v1/invoices/{invoice.id}/taxes",
    ...
    assert line.total_tax_amount.amount == Decimal("0.00")
    assert line.amount.amount == Decimal("10.00")


def test_apply_invoice_tax_invoice_not_found(api_client, user, account):
    tax_rate = TaxRateFactory(account=account)

    api_client.force_login(user)
    api_client.force_account(account)
    response = api_client.post(
        f"/api/v1/invoices/{uuid.uuid4()}/taxes",
        {"tax_rate_id": str(tax_rate.id)},
    )
...
def test_apply_invoice_tax_requires_account(api_client, user):
    api_client.force_login(user)
    response = api_client.post(
        f"/api/v1/invoices/{uuid.uuid4()}/taxes",
...
def test_apply_invoice_tax_non_draft_invoice(api_client, user, account, status):
...
def test_apply_invoice_tax_tax_rate_not_found(api_client, user, account):
...
def test_apply_invoice_tax_duplicate(api_client, user, account):
    invoice = InvoiceFactory(account=account)
    tax_rate = TaxRateFactory(account=account)
        ...
        "errors": [
            {
                "attr": "tax_rate_id",
                "code": "invalid",
                "detail": "Given tax rate is already applied to this invoice",
            }
        ],
    }


def test_apply_invoice_tax_limit(api_client, user, account):
    invoice = InvoiceFactory(account=account)
        ...
        "errors": [
            {
                "attr": None,
                "code": "invalid",
                "detail": "Maximum number of invoice taxes reached",
            }
        ],
    }


def test_apply_invoice_tax_requires_authentication(api_client, account):
    invoice = InvoiceFactory(account=account)
    tax_rate = TaxRateFactory(account=account)
