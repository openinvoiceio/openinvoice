...
def test_retrieve_coupon(api_client, user, account):
...
def test_retrieve_coupon_not_found(api_client, user, account):
    api_client.force_login(user)
    api_client.force_account(account)

    response = api_client.get(f"/api/v1/coupons/{uuid.uuid4()}")
...
def test_retrieve_coupon_requires_account(api_client, user):
...
def test_retrieve_coupon_rejects_foreign_account(api_client, user, account):
    coupon = CouponFactory()
